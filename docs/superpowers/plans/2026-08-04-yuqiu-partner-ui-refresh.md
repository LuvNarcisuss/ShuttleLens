...
- 经用户确认，纯 WXML/WXSS 视觉调整和生成图片不新增源码文本匹配测试；以既有业务运行时测试前后不回归、导航配置测试和微信开发者工具视觉验收为准。
...
- [ ] **Step 1: 写入失败的导航配置与资源契约测试**

在现有 `tabbar.test.js` 中扩展测试：
...
将同文件原有 tabBar 期望对象更新为含 `iconPath`、`selectedIconPath` 的两项配置，并保持页面顺序不变。测试验证应用实际消费的 JSON 配置和本地资源边界，不匹配 WXSS 源码文本。
...
```powershell
git add wx_app/miniprogram/app.json wx_app/miniprogram/app.wxss wx_app/miniprogram/assets/tabbar wx_app/miniprogram/assets/brand wx_app/tests/pages/tabbar.test.js
...
Run: `cd wx_app && node --import tsx --test tests/pages/tasks-runtime.test.js`

Expected: PASS。纯视觉调整按用户批准的 TDD 例外执行，不添加源码文本匹配测试。

- [ ] **Step 2: 重排 WXML 并实现页面样式**
...
```powershell
git add wx_app/miniprogram/pages/tasks
...
Run: `cd wx_app && node --import tsx --test tests/pages/analyze-runtime.test.js`

Expected: PASS。纯视觉调整按用户批准的 TDD 例外执行。

- [ ] **Step 2: 实现四步卡片、按钮和校准状态样式**
...
```powershell
git add wx_app/miniprogram/pages/analyze
...
Run: `cd wx_app && node --import tsx --test tests/pages/result-runtime.test.js`
...
```powershell
git add wx_app/miniprogram/pages/result
...
Run: `cd wx_app && node --import tsx --test tests/pages/login-runtime.test.js`

Expected: PASS。纯视觉调整按用户批准的 TDD 例外执行。

- [ ] **Step 2: 实现登录页品牌和各步骤状态**
...
```powershell
git add wx_app/miniprogram/pages/login
...
Run: `cd wx_app && node --import tsx --test tests/pages/profile-runtime.test.js tests/pages/profile-edit-runtime.test.js`
...
```powershell
git add wx_app/miniprogram/pages/profile wx_app/miniprogram/pages/profile-edit
